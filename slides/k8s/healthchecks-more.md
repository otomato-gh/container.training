...
- All the manifests that we need are in `~/container.training/kubercoins`

.exercise[

- Browse the Kubernetes manifests:
  ```bash
  cd ~/container.training/kubercoins
  ls
  ```
...
- Let's use [`ab`](https://httpd.apache.org/docs/2.4/programs/ab.html) to send concurrent requests to rng
